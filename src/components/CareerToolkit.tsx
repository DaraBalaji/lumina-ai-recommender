...
import {
  CheckCircle2, Clock3, ExternalLink, FolderKanban, Gauge, Lightbulb, Target,
} from 'lucide-react';
...
type ToolkitSection = 'projects' | 'gaps';
  ...
  return <div className="flex flex-col gap-6 pb-16 pt-6">
    <section className="glass-panel rounded-3xl border border-outline-variant/40 p-6 md:p-8">
      <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
        <div><span className="inline-flex items-center gap-2 rounded-full bg-secondary-container/40 px-3 py-1 text-xs font-semibold text-secondary"><Gauge className="h-3.5 w-3.5" /> Career toolkit</span><h1 className="mt-3 text-2xl font-bold text-primary dark:text-on-primary-fixed">Turn learning into evidence</h1><p className="mt-2 max-w-2xl text-sm text-on-surface-variant dark:text-outline-variant">Organize projects and close skill gaps for {role.title}.</p></div>
