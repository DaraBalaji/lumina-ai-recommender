...
import { bodyOf, getDatabase, idOf, json } from './_lib/server';
...
export default async function handler(request: VercelRequest, response: VercelResponse) {
  ...
  try {
    ...
    if (request.method === 'POST' && payload.action === 'create') {
      const owner = idOf(userId) ? await users.findOne({ _id: idOf(userId) }) : null;
    ...
    if (request.method === 'POST' && payload.action === 'invite') {
      ...
      if (room.members.some((member) => member.userId === friend._id.toString()) || room.requests.some((item) => item.userId === friend._id.toString())) return json(response, 409, { error: 'That learner is already a member or has a pending request.' });
    ...
    if (request.method === 'POST' && payload.action === 'requestJoin') {
      ...
      const requester = idOf(userId) ? await users.findOne({ _id: idOf(userId) }) : null;
      if (room.members.some((member) => member.userId === userId) || room.joinRequests?.some((item) => item.userId === userId)) return json(response, 409, { error: 'You are already a member or have a pending request.' });
    ...
    if (request.method === 'POST' && payload.action === 'message') {
      if (!room.members.some((member) => member.userId === userId && member.status === 'accepted')) return json(response, 403, { error: 'Only accepted members can post messages.' });
      const sender = idOf(userId) ? await users.findOne({ _id: idOf(userId) }) : null;
    ...
    if (request.method === 'PATCH' && payload.action === 'respond') {
      const invite = room.requests.find((item) => item.userId === userId);
    ...
    if (request.method === 'PATCH' && payload.action === 'respondJoinRequest') {
      ...
      const joinRequest = room.joinRequests?.find((item) => item.userId === payload.requestUserId);
